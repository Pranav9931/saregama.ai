...
import { rentalContractClient } from "./lib/rentalContract";
import { mimicClient } from "./lib/mimic";
import { verifyRentalPaymentTx, getConfirmationCount } from "./lib/web3Verify";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  /**
   * Get rental pricing tiers
   */
  app.get("/api/rentals/pricing", async (req, res) => {
    try {
      const pricingTiers = await rentalContractClient.getAllPricingTiers();
      const contractAddress = rentalContractClient.getContractAddress();
  ...
  /**
   * Verify on-chain payment and create rental with Mimic expiration task
   * This endpoint is called after user pays via MetaMask
   */
  app.post("/api/rentals/verify-payment", async (req, res) => {
    try {
      const { rentalId, walletAddress, catalogItemId, txHash, durationDays } = req.body;
      
      if (!rentalId || !walletAddress || !catalogItemId || !txHash || !durationDays) {
        return res.status(400).json({ error: "Missing required fields" });
      }
      ...
      // Verify item is available for rental
      if (!item.masterPlaylistId) {
        return res.status(400).json({ error: "Item not available for rental" });
      }
      ...
      // Get contract address and expected price
      const contractAddress = rentalContractClient.getContractAddress();
      if (!contractAddress) {
        return res.status(500).json({ error: "Rental contract not deployed" });
      }

      const priceEth = await rentalContractClient.getPriceForDuration(durationDays);

      // CRITICAL: Verify transaction on-chain
      console.log(`🔍 Verifying transaction ${txHash} on-chain...`);
      const verification = await verifyRentalPaymentTx(
        txHash,
        rentalId,
        walletAddress,
        contractAddress,
      ...
      // Check confirmation count (require at least 1 confirmation)
      const confirmations = await getConfirmationCount(txHash);
      if (confirmations < 1) {
        return res.status(400).json({ 
          error: "Transaction not confirmed yet. Please wait for block confirmation." 
        });
      }

      console.log(`✅ Transaction verified: ${confirmations} confirmations`);

      // Calculate expiration
      const expiresAt = new Date();
      expiresAt.setDate(expiresAt.getDate() + durationDays);

      // Clone playlist for this rental
      let rentalPlaylistId: string | null = null;
      ...
      if (item.masterPlaylistId && arkivClient.isInitialized()) {
        ...
        const { entityKey } = await arkivClient.cloneEntity(item.masterPlaylistId, expirationSeconds);
        rentalPlaylistId = entityKey;
        console.log(`✅ Cloned playlist for rental ${rentalId}: ${rentalPlaylistId}`);
      }

      // Create Mimic task for automatic expiration
      const webhookUrl = process.env.MIMIC_WEBHOOK_URL || `${process.env.APP_URL || 'http://localhost:5000'}/api/webhooks/mimic/rental-expired`;
      const mimicTaskId = await mimicClient.createRentalExpirationTask(
        rentalId,
        contractAddress,
        expiresAt,
        webhookUrl
      ...
      console.log(`✅ Created Mimic expiration task ${mimicTaskId} for rental ${rentalId}`);

      // Update smart contract with Mimic task ID (optional - for on-chain tracking)
      if (contractAddress) {
        await rentalContractClient.updateMimicTaskId(rentalId, mimicTaskId);
      }
      ...
      console.log(`✅ Created rental ${rental.id} for wallet ${walletAddress}`);
      ...
      console.error("Rental verification error:", error);
      res.status(500).json({ error: "Failed to verify payment and create rental" });
    }
  });
  ...
  /**
   * Mimic webhook endpoint for automatic rental expiration
   * Called by Mimic protocol when rental period ends
   */
  app.post("/api/webhooks/mimic/rental-expired", async (req, res) => {
    try {
      // Verify webhook signature
      const signature = req.headers['x-mimic-signature'] as string;
      const webhookSecret = process.env.MIMIC_WEBHOOK_SECRET;
      
      if (webhookSecret && signature) {
        const payload = JSON.stringify(req.body);
        const expectedSignature = ethers.keccak256(
          ethers.toUtf8Bytes(payload + webhookSecret)
        );
        ...
        console.warn('⚠️  Webhook signature missing but secret is configured');
        return res.status(401).json({ error: "Signature required" });
      }

      const { taskId, rentalId, status } = req.body;
      
      console.log(`📨 Mimic webhook received: taskId=${taskId}, rentalId=${rentalId}, status=${status}`);

      if (!rentalId) {
        return res.status(400).json({ error: "Missing rentalId" });
      }

      // Find rental by Mimic task ID or rental ID
      const rental = await storage.getRentalById(rentalId);
      
      if (!rental) {
        console.warn(`⚠️  Rental not found for ID: ${rentalId}`);
        return res.status(404).json({ error: "Rental not found" });
      }

      // Expire the rental in database
      await storage.expireRental(rentalId);

      // Expire the rental on-chain
      if (rental.contractAddress) {
        await rentalContractClient.expireRental(rentalId);
      }

      console.log(`✅ Rental ${rentalId} expired automatically via Mimic`);

      res.json({
        success: true,
        message: "Rental expired successfully",
        rentalId
      });
    } catch (error) {
      console.error("Mimic webhook error:", error);
