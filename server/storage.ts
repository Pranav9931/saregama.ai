...
export class MemStorage implements IStorage {
  ...
  private seedDemoData() {
    // Create demo catalog items with mock data for testing visualizer
    const demoTracks = [
      {
        id: 'demo-track-1',
        type: 'audio' as const,
        ...
        description: 'An upbeat electronic track perfect for testing the visualizer',
        coverUrl: 'https://picsum.photos/seed/demo-track-1/400/400',
        masterPlaylistId: 'mock_playlist_entity_1',
        masterPlaylistTxHash: '0x123456789abcdef',
        priceEth: '0.001',
        durationSeconds: 180,
        category: 'featured',
      ...
      },
      {
        ...
        coverUrl: 'https://picsum.photos/seed/demo-track-2/400/400',
        masterPlaylistId: 'mock_playlist_entity_2',
        masterPlaylistTxHash: '0xabcdef123456789',
        priceEth: '0.0015',
        durationSeconds: 240,
        category: 'trending',
        createdBy: '0x0000000000000000000000000000000000000001',
      ...
      },
      {
        ...
        artist: 'Folk Band',
        description: 'Peaceful acoustic melodies',
        coverUrl: 'https://picsum.photos/seed/demo-track-3/400/400',
        masterPlaylistId: 'mock_playlist_entity_3',
        masterPlaylistTxHash: '0x789abcdef123456',
        priceEth: '0.0008',
        durationSeconds: 200,
        category: 'new-releases',
        createdBy: '0x0000000000000000000000000000000000000002',
        createdAt: new Date(),
      },
    ];

    demoTracks.forEach((track) => {
      this.catalogItems.set(track.id, track as CatalogItem);
      
      // Create mock chunks for each track
      for (let i = 0; i < 3; i++) {
        const chunkId = `demo-chunk-${track.id}-${i}`;
        const chunk: CatalogChunk = {
          id: chunkId,
          catalogItemId: track.id,
          sequence: i,
          arkivEntityId: `mock_chunk_entity_${track.id}_${i}`,
          arkivTxHash: `0x${Math.random().toString(16).slice(2, 66)}`,
          metadataEntityId: `mock_metadata_${track.id}_${i}`,
          nextChunkId: i < 2 ? `mock_chunk_entity_${track.id}_${i + 1}` : null,
          sizeBytes: 80000,
          expiresAt: null,
          createdAt: new Date(),
        };
        this.catalogChunks.set(chunkId, chunk);
      }
    });

    console.log('✅ Seeded demo data: 3 tracks with chunks');
  }
